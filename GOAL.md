...
### Generated render-command ABI

The compact production subset now covers 52 structures and 375 members. Typed
nested writers add 2D rectangles, color/depth clear values, dynamic rendering
attachments/info, and render-pass begin records. All shared dynamic and legacy
render-command paths use generated sizes, tags, and member offsets without hot
allocation.

This corrected the old dynamic attachment packing, which placed the nested
`VkClearValue` at byte 56 instead of its generated byte 52 and shifted color and
depth data. Registry determinism and the full OpenGL/Vulkan graph suite pass;
Vulkan depth, resolve, MRT, subpass, and 1,001-frame warmed paths remain
`live=0`, with X11 Vulkan, Abla-only, and direct runtime linkage also passing.

The compiler launcher in `../ablac/tools/run-limited-compiler.sh` accepts an
explicit `ABLA_COMPILER_PAYLOAD` for non-destructive validation against a known
compiler while another compiler payload is under development. This checkpoint
was validated with the last known-good payload because unrelated uncommitted
LLVM work had rebuilt the default payload mid-run; those LLVM files were left
...
The LLVM call retains the exact native return width and widens `F32` once to
Abla `f64`; generated raw methods store the resulting `f64` bits in the
caller-owned result cell. All 76 compiler tests passed, including the encoded
float-result regression, followed by a byte-identical pure-Abla self-rebuild
and working native child.

One pure-Abla normalization batch now classifies `GLhalfNV` as a 16-bit lane,
