...
cd "$compiler_root"
ABLA_MAX_MEMORY_MB=${ABLA_HDR_PBR_TEST_MEMORY_MB:-8192} \
    ABLA_SYSROOT="$compiler_root" "$compiler" \
    build --project "$project_root/examples/hdr-pbr" \
    -o "$output_directory/hdr-pbr"

for backend in opengl vulkan auto; do
    xvfb-run -a -s "-screen 0 800x600x24" \
        "$output_directory/hdr-pbr" "$backend"
done
