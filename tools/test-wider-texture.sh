...
cd "$compiler_root"
ABLA_MAX_MEMORY_MB=${ABLA_WIDER_TEXTURE_TEST_MEMORY_MB:-8192} \
    ABLA_SYSROOT="$compiler_root" "$compiler" \
    build --project "$project_root/tests/wider_texture" \
    -o "$output_directory/wider-texture" --no-cache
...
for backend in opengl vulkan auto; do
    ...
    if [[ $status -ne 42 ]]; then
        printf 'Abla wider %s texture test returned %s, expected 42\n' \
            "$backend" "$status" >&2
        exit 1
    fi
done
