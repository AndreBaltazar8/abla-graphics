...
cd "$compiler_root"
ABLA_MAX_MEMORY_MB=${ABLA_TEXTURE_POOL_TEST_MEMORY_MB:-6144} \
    ABLA_SYSROOT="$compiler_root" "$compiler" \
    build "$project_root/tests/texture_pool/main.ab" \
    -o "$output_directory/texture-pool" --no-cache
...
for backend in opengl vulkan auto; do
    ...
    if [[ $status -ne 42 ]]; then
        printf 'Abla %s texture-pool test returned %s, expected 42\n' \
            "$backend" "$status" >&2
        exit 1
    fi
done
