...
`void(pointer,i32,pointer)`, `i32(pointer,i64,pointer)`, and
`i32(pointer,i64,pointer,pointer)` result/query families, plus
`void(pointer,i64,i32)` and `void(pointer,i64,i32,i32)` command families.
Exact 64-bit synchronization additionally covers `void(pointer,i64,i64)`,
`void(pointer,i64,i64,i32)`, `i32(pointer,pointer,i64)`, and
`i32(pointer,i32,pointer,i64)`. Device-address return, buffer binding,
...
all 2,892 OpenGL entries and all 842 pinned Vulkan entries. Vulkan's former 50
one-off layouts and nine platform-scalar boundaries are fully lowered with
explicit native widths. OpenGL preserves `GLhandleARB` as a generated platform
kind: it resolves to pointer returns and parameters on Apple and unsigned
32-bit returns/integer parameters elsewhere. Capability gates, live extension
...
two-texture procedural lighting pass. General deterministic sampled-expression
lowering now composes `texture(sampler, coordinateExpression)` results with
typed coordinates, locals, swizzles, arithmetic, and built-ins across up to
eight mixed set-zero `sampler2D`, `sampler2DArray`, `samplerCube`, and
`sampler3D` bindings. The deferred and wider-sampling applications exercise
computed `vec2`/`vec3` coordinates, sampled locals, vector addition, `mix`, and
...
`textureSize` now returns dimension-exact signed vectors for 2D, 2D-array,
...
baseline sequential JPEG decoding now shares that path for grayscale or one
interleaved YCbCr scan, with subsampling, restart, integer-IDCT, and malformed-
stream validation. FLOAT `VEC3`
...
backends. Ten Khronos material extensions now have typed factors, colors,
...
models remain open. The ordinary glTF/JFIF grayscale and interleaved three-
component baseline JPEG subset is implemented; non-interleaved, four-component,
progressive, arithmetic, lossless, and hierarchical forms remain explicit
future extensions.
...
12. headless image generation (delivered by `examples/headless-image`, with
    display-free GPU color generation and byte-identical golden PPM export on
    OpenGL and Vulkan);
