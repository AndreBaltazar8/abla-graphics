...
- `examples/narrow-input`: one interleaved `vec2` vertex attribute is
  interpolated through a typed `vec2` fragment location and composed into a
  `vec4` color, proving exact pixels, stable handles, and zero-growth repeated
  offscreen and presented frames on OpenGL and Vulkan;
