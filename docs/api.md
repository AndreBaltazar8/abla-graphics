...
by `registry/driver-vulkan-structures.txt`. It currently covers 43 structures
and 335 members, while the complete 7,406-member schema remains opt-in. Device
...
buffer-image regions, image copies, blits, and resolves use generated layouts.
Setup-only generic access is available, while hot typed builders consume
