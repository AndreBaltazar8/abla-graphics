...
- Nix-built application linkage: the development shell adds concrete Vulkan,
  ...
  silent.
  Sample 75 is the display-free headless-image proof: explicit OpenGL and
  validation-enabled Vulkan independently produce the same four GPU-derived
  RGBA8 colors and byte-identical 16x16 PPM with pinned SHA-256
  ...
  The first ledger-driven batch credits 10 already-live OpenGL commands and 13
  already-live Vulkan commands while leaving shape-only probes unclaimed; the
  command ledgers now classify 119/2,892 and 126/842 entries respectively.
...
### Generated render-command layouts

The production subset now contains 52 Vulkan structures and 375 members.
Nested typed writers cover 2D offsets/extents/rectangles, color and
depth/stencil clear values, dynamic rendering attachments/info, and render-pass
begin records. Dynamic rendering and all shared legacy render-pass command
paths now use generated offsets and caller-owned scratch storage.

Registry layout generation corrected a real dynamic-rendering ABI error: the
old code treated `VkRenderingAttachmentInfo.clearValue` as beginning at byte 56
instead of the generated byte 52, shifting RGBA and depth values. The full
graph/render suite passes on Vulkan, including depth, resolve, MRT, subpass, and
1,001-frame warmed paths at `live=0`; X11 Vulkan, Abla-only, and direct runtime
linkage also pass with `unresolved=0`.

### Generated render-pass and framebuffer creation

The production subset now contains 57 Vulkan structures and 414 members.
Attachment descriptions/references, subpass descriptions, render-pass creation,
and framebuffer creation are generated for compatible passes, offscreen
targets, and surfaced legacy rendering. Exact generated sizes also replace the
old padded attachment arrays and handwritten 36-byte strides.
