...
- Full GLSL 4.60 grammar validation/reflection or SPIR-V emission. The current
  ...
  raster postfix IR now emits fragment `vec4` push-member/literal and scalar
  literal expressions with parenthesized multiplication, addition, and
  subtraction directly to deterministic SPIR-V, including both operand orders
  of `OpVectorTimesScalar`; fixed fragment push word tables have been removed.
  Typed mismatch rejection, precedence, repeat-emission, exact real-driver
  pixels, stable handles, and zero-growth frames are verified. Reflected scalar
  members, division, interface inputs, functions, and control flow remain
  upcoming.
